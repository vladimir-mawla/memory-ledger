...
/**
 * INVARIANT: `lib/decay/**` NEVER reaches an LLM, the network, or a
 * Node built-in — `sim-plan.md` §A.2's structural answer to "you're just
 * asking a model to guess and calling it a simulation": "`lib/decay/**`
 ...
 *     clean from `lib/decay/adapter.ts`, because `../../node_modules/
 ...
 *     decay/`, `lib/contracts/`), with `root + sep` prefix comparison
 *     so `lib/decay-experimental` cannot pass by string-prefix
 *     coincidence with `lib/decay`.
 *   - ROUND 2 (independent verification): the round-1 fix checked the
 *     TEXTUAL resolved path, never dereferencing a symlink — a real
 *     symlink placed inside `lib/decay/`, pointing outside the
...
/** The closed list of directories a `lib/decay/**` source file may resolve an import into — itself, and the one frozen dependency it is allowed to use. See the file header's "FIX HISTORY, PATH-CONTAINMENT SIDE" for why containment against THIS list, not specifier syntax, is the actual property being checked. */
const ALLOWED_ROOTS: readonly string[] = [DECAY_ROOT, CONTRACTS_ROOT];
/** The SAME roots, realpath'd once at module load — comparing a realpath'd candidate against un-normalized `ALLOWED_ROOTS` would misfire the moment the repository itself sits under a symlink (common on macOS, where `/tmp` is itself a symlink to `/private/tmp`). Computed eagerly, not defensively wrapped in try/catch: `lib/decay/` and `lib/contracts/` not existing at all would mean this very test file couldn't have been found to run in the first place. */
...
const FROM_DECAY = join(DECAY_ROOT, "adapter.ts");
...
function analyzeSource(source: string): { specifiers: readonly FoundSpecifier[]; bareFetchCalls: readonly FoundBareFetchCall[] } {
  const dir = mkdtempSync(join(tmpdir(), "memory-ledger-decay-arch-scan-"));
...
function scan(): { specifierOffenders: SpecifierOffender[]; fetchOffenders: FetchOffender[]; parseOffenders: ParseOffender[] } {
  ...
  for (const file of listNonTestSourceFiles(DECAY_ROOT)) {
...
describe("lib/decay/** never reaches an LLM, the network, or a Node built-in", () => {
  it("every import/require specifier in non-test source is relative AND resolves inside lib/decay/ or lib/contracts/ — no bare package, no node: built-in, no absolute path, no relative escape into node_modules or a sibling milestone", () => {
    ...
    if (specifierOffenders.length > 0) {
      ...
      throw new Error(
        `lib/decay/** may only import a specifier that resolves inside lib/decay/ or lib/contracts/ — found ` +
  ...
  it("no non-test source file calls the bare global fetch(...)", () => {
    ...
    if (fetchOffenders.length > 0) {
      ...
      throw new Error(`lib/decay/** may never call the global fetch() — found ${fetchOffenders.length} call(s):\n${report}`);
    }
    expect(fetchOffenders).toEqual([]);
  });

  it("every non-test source file under lib/decay/** parses as valid TypeScript — a file this guard cannot parse is an automatic offender, never silently treated as clean (round 5)", () => {
    const { parseOffenders } = scan();
    if (parseOffenders.length > 0) {
      const report = parseOffenders.map((o) => `${o.file}: ${o.message}`).join("\n");
      throw new Error(
        `lib/decay/** contains ${parseOffenders.length} file(s) this guard could not parse as valid TypeScript ` +
  ...
  describe("sanity: the containment rule itself", () => {
    it("accepts relative specifiers that resolve inside lib/decay/ itself", () => {
      expect(resolvesInsideAllowedRoots(FROM_DECAY, "./decay.js")).toBe(true);
      expect(resolvesInsideAllowedRoots(FROM_DECAY, "./index.js")).toBe(true);
    });
    ...
    it("accepts a relative specifier that resolves into lib/contracts/ — the one frozen dependency this milestone is allowed to use", () => {
      expect(resolvesInsideAllowedRoots(FROM_DECAY, "../contracts/index.js")).toBe(true);
    });

    it("rejects every non-relative (bare/absolute) shape outright, with no enumeration needed", () => {
      expect(resolvesInsideAllowedRoots(FROM_DECAY, "node:fs")).toBe(false);
      expect(resolvesInsideAllowedRoots(FROM_DECAY, "fs")).toBe(false);
      expect(resolvesInsideAllowedRoots(FROM_DECAY, "openai")).toBe(false);
      expect(resolvesInsideAllowedRoots(FROM_DECAY, "@anthropic-ai/sdk")).toBe(false);
      expect(resolvesInsideAllowedRoots(FROM_DECAY, "node-fetch")).toBe(false);
      expect(resolvesInsideAllowedRoots(FROM_DECAY, "react")).toBe(false);
      ...
      expect(resolvesInsideAllowedRoots(FROM_DECAY, "some-llm-sdk-invented-tomorrow")).toBe(false);
    });
    ...
    it("EXPLOIT REGRESSION (HIGH-2, independent verification): a SYNTACTICALLY relative specifier that resolves outside both allowed roots, into the real node_modules/, is rejected", () => {
      ...
      const resolved = resolve(dirname(FROM_DECAY), specifier);
      ...
      expect(resolved.startsWith(DECAY_ROOT + sep)).toBe(false);
      expect(resolved.startsWith(CONTRACTS_ROOT + sep)).toBe(false);

      expect(resolvesInsideAllowedRoots(FROM_DECAY, specifier)).toBe(false);
    });

    it("rejects an escape into a SIBLING milestone's own frozen directory — specifically lib/contradiction/**, M4's concurrently-built engine this milestone must not import, stub, or wait for", () => {
      expect(resolvesInsideAllowedRoots(FROM_DECAY, "../contradiction/contradict.js")).toBe(false);
    });

    it("does not fall for a sibling directory that merely shares lib/decay's name as a string prefix (e.g. a hypothetical lib/decay-experimental/)", () => {
      const fakeSiblingFile = join(REPO_ROOT, "lib", "decay-experimental", "evil.ts");
      // Importing FROM inside a same-prefixed sibling should not be
      // treated as importing from inside lib/decay/ itself — this
    ...
    it("EXPLOIT REGRESSION (HIGH-2, ROUND 2 — independent verification): a REAL symlink inside lib/decay/, pointing at a directory outside the repository, is rejected once dereferenced", () => {
      ...
      // lib/decay/, but whose REAL target is not.
      const symlinkName = "__zz_symlink_regression_do_not_commit__";
      const symlinkPath = join(DECAY_ROOT, symlinkName);
      const externalTarget = mkdtempSync(join(tmpdir(), "memory-ledger-decay-symlink-attack-"));
      ...
      try {
        ...
        expect(real.startsWith(DECAY_ROOT + sep)).toBe(false);
    ...
    it("sanity: a symlinked DIRECTORY that points somewhere legitimate (still outside both roots, but not malicious) is treated identically — this check is about containment, not intent", () => {
      ...
      const symlinkPath = join(DECAY_ROOT, symlinkName);
      const externalTarget = mkdtempSync(join(tmpdir(), "memory-ledger-decay-symlink-benign-"));
      writeFileSync(join(externalTarget, "harmless.js"), "export default 2;\n");

      try {
        symlinkSync(externalTarget, symlinkPath, "dir");
        expect(resolvesInsideAllowedRoots(FROM_DECAY, `./${symlinkName}/harmless.js`)).toBe(false);
      } finally {
        rmSync(symlinkPath, { force: true, recursive: true }); // recursive is safe here — Node never dereferences a symlink for removal, it just unlinks the symlink entry itself; recursive is only needed because rmSync's own directory-ness check sees a symlink-to-directory and otherwise refuses.
        rmSync(externalTarget, { recursive: true, force: true });
      }
    ...
    it("FAIL CLOSED (round-2 fix, point 1): a specifier whose target does not exist under any name — leaf or enclosing directory — is rejected, not waved through", () => {
      ...
      expect(resolvesInsideAllowedRoots(FROM_DECAY, "./totally-nonexistent-dir/also-nonexistent.js")).toBe(false);
    });
    ...
    it("does not regress the legitimate .js-specifier-to-.ts-file convention this codebase actually uses (the directory-level fallback's whole reason to exist)", () => {
      ...
      expect(existsSync(join(DECAY_ROOT, "decay.js"))).toBe(false); // confirms the premise: the literal name really is absent.
      expect(existsSync(join(DECAY_ROOT, "decay.ts"))).toBe(true); // the real file, under a different extension.
      expect(resolvesInsideAllowedRoots(FROM_DECAY, "./decay.js")).toBe(true);
    });
  });
  ...
  describe("teeth: the same evasions that defeated the sibling's line-anchored guard, plus the round-4 regex bypass, confirmed caught here", () => {
    it("a multi-line named import — the exact Prettier shape that defeated decision-engine's original matcher", () => {
      ...
      expect(resolvesInsideAllowedRoots(FROM_DECAY, specifiers[0]!.specifier)).toBe(false);
    });
    ...
    it("a dynamically COMPUTED import specifier is now flagged outright — previously an explicitly disclosed, undetectable gap; the AST sees the call is an import regardless of its argument's shape", () => {
      ...
      expect(resolvesInsideAllowedRoots(FROM_DECAY, "specifierVar")).toBe(false);
    });
  });
  ...
  describe("false-positive discipline: only real specifiers/calls, never identifiers, comments, or unrelated strings", () => {
    it("does NOT flag a relative import whose filename merely contains the word 'fetch'", () => {
      ...
      expect(resolvesInsideAllowedRoots(FROM_DECAY, specifiers[0]!.specifier)).toBe(true);
    });
    ...
    it("does NOT overtighten: a legitimate relative import specifier written inside a benign interpolation is still accepted, not merely un-flagged", () => {
      const source = "const mod = `${(() => import(`./index.js`))()}`;";
      const { specifiers } = analyzeSource(source);
      expect(specifiers).toContainEqual({ specifier: "./index.js", line: 1 });
      expect(resolvesInsideAllowedRoots(FROM_DECAY, "./index.js")).toBe(true);
    });
    ...
    it("does not false-positive on ordinary relative imports already used throughout this milestone", () => {
      const specifiers = ["./decay.js", "../contracts/index.js", "./index.js", "../contracts/confidence.js"];
      for (const specifier of specifiers) {
        expect(resolvesInsideAllowedRoots(FROM_DECAY, specifier)).toBe(true);
      }
    });
  });

  it("sanity: the scan actually walked real files under lib/decay/, and excludes __tests__", () => {
    const files = listNonTestSourceFiles(DECAY_ROOT);
    expect(files.length).toBeGreaterThanOrEqual(2);
    expect(files.some((f) => f.endsWith("decay.ts"))).toBe(true);
    expect(files.some((f) => f.includes("__tests__"))).toBe(false);
  });

  it("sanity: this repo's OWN lib/decay/** source passes all three checks right now (a true positive on the real codebase, not just synthetic fixtures)", () => {
  ...
  describe("EXPLOIT REGRESSION (independent verification, round 5): syntax errors defeated the real parser's error recovery the same way earlier rounds defeated the tokenizer — the bug class MOVED, not closed, until diagnostics are checked", () => {
    ...
    it("does NOT overtighten: a legitimate relative re-export is still accepted, not merely un-flagged", () => {
      ...
      expect(resolvesInsideAllowedRoots(FROM_DECAY, "./helper.js")).toBe(true);
    });
  });
});
