 ...
 *
 * Copied as a mechanism from shadow-run's own app/milestones.test.ts (which
 * copied it from decision-engine): this guard exists because of a specific,
 * observed failure on that project's own history — the landing page
 * announced "Milestone M2 of 9" for the whole of M3 and M4, on a public
 * URL, because the number lived in prose. That was fixed by deriving it
 * from a module, which is what app/milestones.ts is here.
 *
 ...
 *
 * THE PARSER BELOW IS COPIED VERBATIM (mechanism, not text) FROM
 * shadow-run's OWN, ALREADY-HARDENED VERSION, NOT REDISCOVERED HERE:
 * shadow-run's own history records that this exact parser took six rounds
 * and five demonstrated bypasses before it held — a decoy pill-ish span
 * elsewhere in the row, a second real pill winning by match order in
 * either direction, stray leading/trailing text sharing the cell, and
 * three separate holes in "tolerate a nested decorative element" (a nested
 * decoy pill, an unrelated nested tag whose own text looked like a status,
 * and a nested tag whose quoted attribute contained '>' and broke a
 * generic tag-depth walker). The lesson that history draws, and that this
 * file inherits rather than re-learns: parsing flexible HTML is the wrong
 * shape. The status cell must match ONE canonical form exactly, with no
 * nested elements, or it is rejected outright — no tolerance is added here
 ...
 *
 * Anchored to the row's LAST `<td>` (the table's own header names it
 * "Status") and requires the whole trimmed cell to be exactly one pill
 ...
 * otherwise.
 *
 * This is a single anchored regex on purpose. shadow-run's own history
 * (see this file's header) went through three earlier rounds of "parse the
 * structure, tolerating a nested element" — a `<span>`-nesting depth walk,
 * then a generic-tag depth walk on top of it, plus separate self-closing
 * detection — and each round's added tolerance grew its own hole (a nested
 * decoy pill; an unrelated nested tag whose own text read as a status; a
 * quoted attribute containing '>' that broke the tag-matching regex's
 * notion of where a tag ends). NOBODY HAS ASKED FOR A DECORATIVE NESTED
 * ELEMENT in this project either — it was speculative tolerance on the
 * sibling project, and it cost three straight rounds of holes there. So
 * this file starts where that history ended: nested elements of any kind —
 * empty, whitespace-only, or otherwise — are simply not part of the shape
 * this parser accepts, full stop.
 *
 * LOAD-BEARING, every one of them a demonstrated bypass on the sibling
 * project, not incidental strictness:
 *   - No nested elements, at all, under any circumstances. See above.
 *   - Exactly ONE element must span the WHOLE trimmed cell (the `^`/`$`
 *     anchors), with nothing else beside it — rejects a second pill
 *     winning by appearing first, in either order.
 ...
 *     an attribute is exactly the vector the quoted-'>' bypass came
 *     through on the sibling project.
 *   - The extracted status text must be lowercase. This file's own
 *     convention (mirroring .genesis/DONE.html) is all-lowercase status
 *     words; a stray capital is a typo worth catching, not a shape this
 *     parser should shrug at.
 ...
 *
 * CALIBRATION — real, zero-risk edits this parser must NOT reject:
...
describe("pillsInHtml (the drift guard's own parser)", () => {
  it("reads the row's real trailing status pill, not a decoy pill-ish span in the title cell", () => {
    // Verified reproduction of the bug this parser must not have: a decoy
    // span earlier in the row's free-text title cell (class starts with
    // "pill" but isn't the status pill) must not be what gets read. The
    // rendered row's real status, in its last cell, is "todo" — this must
  ...
  // The three demonstrated nested-element bypasses from shadow-run's own
  // history (see this file's header), run against THIS project's parser —
  // proving the "no nested elements, ever" rule actually holds here too,
  // not merely inherited by copying the comment.
  it("attack (demonstrated bypass on the sibling project): a nested decoy pill — throws", () => {
    const html = row('<span class="pill todo"><span class="pill fake">wrongstate</span></span>');
    expect(() => pillsInHtml(html)).toThrow(/malformed status cell/);
  });

  it("attack (demonstrated bypass on the sibling project): an unrelated nested <b> whose text looks like a status — throws", () => {
  ...
  it("attack (demonstrated bypass on the sibling project, via attribute quoting): a nested tag with a quoted attribute containing '>' — throws", () => {
    // The bypass that ended nested-element support entirely on the sibling
    // project: a generic tag-matching regex's `[^>]*` has no concept of a
    // quoted attribute, so it stopped at the '>' inside `data-x="a/>done"`,
    // mistook the truncated match's trailing "/>" for a real self-close,
    // and read the nested text as direct content. This parser has no
    // tag-depth-walking machinery left to have that hole — it simply
    // rejects the '<' at all.
