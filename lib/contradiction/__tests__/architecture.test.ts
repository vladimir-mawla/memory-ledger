 ...
 * THIS MILESTONE'S OWN BRIEF: "Copy [it] — it took three rejection rounds
 * and seven bypasses to harden, uses the real TypeScript compiler ... and
 * fails closed on unparseable files. Do not write your own scanner; do not
 * rediscover those seven bypasses." Only the directory-specific constants
 * (`SIMULATE_ROOT` → `CONTRADICTION_ROOT`, `FROM_ADAPTER` → `FROM_CONTRADICT`)
 * and the file-count/filename sanity checks at the bottom are changed for
 * this milestone's own tree. EVERY ROUND OF FIX HISTORY BELOW IS KEPT
 * VERBATIM, UNMODIFIED, BECAUSE IT IS THE PART THE BRIEF SAYS NOT TO
 * REDISCOVER — re-reading it before trusting this file is still the right
 * instinct, exactly as it was in `shadow-run`.
 *
 * INVARIANT: `lib/contradiction/**` NEVER reaches an LLM, the network, or a
 * Node built-in — `memory-plan.md` §5's own structural requirement:
 * "Contradiction detection is never judged by a model... no LLM call sits
 * anywhere in `lib/contradiction/**` or `lib/decay/**`, enforced the same
 * way `shadow-run` enforces it for `lib/simulate/**`: a grep-based
 * architecture test asserting zero LLM-client/network imports." The build
 * brief additionally asks for this to survive ordinary reformatting, citing
 * two precedents from this account's own history — read both before
 * trusting any check here:
 *
...
/** The closed list of directories a `lib/contradiction/**` source file may resolve an import into — itself, and the one frozen dependency this milestone is allowed to use. See the file header's "FIX HISTORY, PATH-CONTAINMENT SIDE" for why containment against THIS list, not specifier syntax, is the actual property being checked. */
const ALLOWED_ROOTS: readonly string[] = [CONTRADICTION_ROOT, CONTRACTS_ROOT];
/** The SAME roots, realpath'd once at module load — comparing a realpath'd candidate against un-normalized `ALLOWED_ROOTS` would misfire the moment the repository itself sits under a symlink (common on macOS, where `/tmp` is itself a symlink to `/private/tmp`). Computed eagerly, not defensively wrapped in try/catch: `lib/contradiction/` and `lib/contracts/` not existing at all would mean this very test file couldn't have been found to run in the first place. */
const REAL_ALLOWED_ROOTS: readonly string[] = ALLOWED_ROOTS.map((root) => realpathSync(root));
/** A representative real file location, used throughout this file's synthetic (no-real-file-needed) specifier checks below — `resolve()` is pure path arithmetic and does not require `contradict.ts` to be the file actually being checked. */
const FROM_CONTRADICT = join(CONTRADICTION_ROOT, "contradict.ts");
 ...
 * same discipline this codebase's own contracts-side error classes
 * (`NonPlainDataError`) already establish elsewhere.
 */
...
function analyzeSource(source: string): { specifiers: readonly FoundSpecifier[]; bareFetchCalls: readonly FoundBareFetchCall[] } {
  const dir = mkdtempSync(join(tmpdir(), "memory-ledger-arch-scan-"));
...
function scan(): { specifierOffenders: SpecifierOffender[]; fetchOffenders: FetchOffender[]; parseOffenders: ParseOffender[] } {
  ...
  for (const file of listNonTestSourceFiles(CONTRADICTION_ROOT)) {
...
describe("lib/contradiction/** never reaches an LLM, the network, or a Node built-in", () => {
  it("every import/require specifier in non-test source is relative AND resolves inside lib/contradiction/ or lib/contracts/ — no bare package, no node: built-in, no absolute path, no relative escape into node_modules or a sibling milestone", () => {
    ...
    if (specifierOffenders.length > 0) {
      ...
      throw new Error(
        `lib/contradiction/** may only import a specifier that resolves inside lib/contradiction/ or lib/contracts/ — found ` +
  ...
  it("no non-test source file calls the bare global fetch(...)", () => {
    ...
    if (fetchOffenders.length > 0) {
      ...
      throw new Error(`lib/contradiction/** may never call the global fetch() — found ${fetchOffenders.length} call(s):\n${report}`);
    }
    expect(fetchOffenders).toEqual([]);
  });

  it("every non-test source file under lib/contradiction/** parses as valid TypeScript — a file this guard cannot parse is an automatic offender, never silently treated as clean (round 5)", () => {
    const { parseOffenders } = scan();
    if (parseOffenders.length > 0) {
      const report = parseOffenders.map((o) => `${o.file}: ${o.message}`).join("\n");
      throw new Error(
        `lib/contradiction/** contains ${parseOffenders.length} file(s) this guard could not parse as valid TypeScript ` +
  ...
  describe("sanity: the containment rule itself", () => {
    it("accepts relative specifiers that resolve inside lib/contradiction/ itself", () => {
      expect(resolvesInsideAllowedRoots(FROM_CONTRADICT, "./value-comparator.js")).toBe(true);
      expect(resolvesInsideAllowedRoots(FROM_CONTRADICT, "./contradiction-check.js")).toBe(true);
    });
    ...
    it("accepts a relative specifier that resolves into lib/contracts/ — the one frozen dependency this milestone is allowed to use", () => {
      expect(resolvesInsideAllowedRoots(FROM_CONTRADICT, "../contracts/index.js")).toBe(true);
    });

    it("rejects every non-relative (bare/absolute) shape outright, with no enumeration needed", () => {
      expect(resolvesInsideAllowedRoots(FROM_CONTRADICT, "node:fs")).toBe(false);
      expect(resolvesInsideAllowedRoots(FROM_CONTRADICT, "fs")).toBe(false);
      expect(resolvesInsideAllowedRoots(FROM_CONTRADICT, "openai")).toBe(false);
      expect(resolvesInsideAllowedRoots(FROM_CONTRADICT, "@anthropic-ai/sdk")).toBe(false);
      expect(resolvesInsideAllowedRoots(FROM_CONTRADICT, "node-fetch")).toBe(false);
      expect(resolvesInsideAllowedRoots(FROM_CONTRADICT, "react")).toBe(false);
      ...
      expect(resolvesInsideAllowedRoots(FROM_CONTRADICT, "some-llm-sdk-invented-tomorrow")).toBe(false);
    });
    ...
    it("EXPLOIT REGRESSION (HIGH-2, independent verification): a SYNTACTICALLY relative specifier that resolves outside both allowed roots, into the real node_modules/, is rejected", () => {
      ...
      const resolved = resolve(dirname(FROM_CONTRADICT), specifier);
      ...
      expect(resolved.startsWith(CONTRADICTION_ROOT + sep)).toBe(false);
      expect(resolved.startsWith(CONTRACTS_ROOT + sep)).toBe(false);

      expect(resolvesInsideAllowedRoots(FROM_CONTRADICT, specifier)).toBe(false);
    });

    it("rejects an escape into a SIBLING milestone's own frozen directory (e.g. lib/decay/, M3 — unbuilt on this branch but still a forbidden escape target), not just node_modules", () => {
      expect(resolvesInsideAllowedRoots(FROM_CONTRADICT, "../decay/interpret.js")).toBe(false);
    });

    it("does not fall for a sibling directory that merely shares lib/contradiction's name as a string prefix (e.g. a hypothetical lib/contradiction-experimental/)", () => {
      const fakeSiblingFile = join(REPO_ROOT, "lib", "contradiction-experimental", "evil.ts");
      // Importing FROM inside a same-prefixed sibling should not be
      // treated as importing from inside lib/contradiction/ itself — this
    ...
    it("EXPLOIT REGRESSION (HIGH-2, ROUND 2 — independent verification): a REAL symlink inside lib/contradiction/, pointing at a directory outside the repository, is rejected once dereferenced", () => {
      ...
      // lib/contradiction/, but whose REAL target is not.
      const symlinkName = "__zz_symlink_regression_do_not_commit__";
      const symlinkPath = join(CONTRADICTION_ROOT, symlinkName);
      const externalTarget = mkdtempSync(join(tmpdir(), "memory-ledger-symlink-attack-"));
      ...
      try {
        ...
        // lib/contradiction/ at all — it is one path segment inside it.
        // The textual check alone would (correctly, on its own terms)
        // pass this; the symlink is what makes the real target different.
        expect(resolved.startsWith(CONTRADICTION_ROOT + sep)).toBe(true);
        ...
        expect(real.startsWith(CONTRADICTION_ROOT + sep)).toBe(false);
    ...
    it("sanity: a symlinked DIRECTORY that points somewhere legitimate (still outside both roots, but not malicious) is treated identically — this check is about containment, not intent", () => {
      ...
      const symlinkPath = join(CONTRADICTION_ROOT, symlinkName);
      const externalTarget = mkdtempSync(join(tmpdir(), "memory-ledger-symlink-benign-"));
      writeFileSync(join(externalTarget, "harmless.js"), "export default 2;\n");

      try {
        symlinkSync(externalTarget, symlinkPath, "dir");
        expect(resolvesInsideAllowedRoots(FROM_CONTRADICT, `./${symlinkName}/harmless.js`)).toBe(false);
      } finally {
        rmSync(symlinkPath, { force: true, recursive: true }); // recursive is safe here — Node never dereferences a symlink for removal, it just unlinks the symlink entry itself; recursive is only needed because rmSync's own directory-ness check sees a symlink-to-directory and otherwise refuses.
        rmSync(externalTarget, { recursive: true, force: true });
      }
    ...
    it("FAIL CLOSED (round-2 fix, point 1): a specifier whose target does not exist under any name — leaf or enclosing directory — is rejected, not waved through", () => {
      ...
      expect(resolvesInsideAllowedRoots(FROM_CONTRADICT, "./totally-nonexistent-dir/also-nonexistent.js")).toBe(false);
    });
    ...
    it("does not regress the legitimate .js-specifier-to-.ts-file convention this codebase actually uses (the directory-level fallback's whole reason to exist)", () => {
      ...
      expect(existsSync(join(CONTRADICTION_ROOT, "value-comparator.js"))).toBe(false); // confirms the premise: the literal name really is absent.
      expect(existsSync(join(CONTRADICTION_ROOT, "value-comparator.ts"))).toBe(true); // the real file, under a different extension.
      expect(resolvesInsideAllowedRoots(FROM_CONTRADICT, "./value-comparator.js")).toBe(true);
    });
  });
  ...
  describe("teeth: the same evasions that defeated the sibling's line-anchored guard, plus the round-4 regex bypass, confirmed caught here", () => {
    it("a multi-line named import — the exact Prettier shape that defeated decision-engine's original matcher", () => {
      ...
      expect(resolvesInsideAllowedRoots(FROM_CONTRADICT, specifiers[0]!.specifier)).toBe(false);
    });
    ...
    it("a dynamically COMPUTED import specifier is now flagged outright — previously an explicitly disclosed, undetectable gap; the AST sees the call is an import regardless of its argument's shape", () => {
      ...
      expect(resolvesInsideAllowedRoots(FROM_CONTRADICT, "specifierVar")).toBe(false);
    });
  });
  ...
  describe("false-positive discipline: only real specifiers/calls, never identifiers, comments, or unrelated strings", () => {
    it("does NOT flag a relative import whose filename merely contains the word 'fetch'", () => {
      ...
      expect(resolvesInsideAllowedRoots(FROM_CONTRADICT, specifiers[0]!.specifier)).toBe(true);
    });
    ...
    it("does NOT overtighten: a legitimate relative import specifier written inside a benign interpolation is still accepted, not merely un-flagged", () => {
      const source = "const mod = `${(() => import(`./value-comparator.js`))()}`;";
      const { specifiers } = analyzeSource(source);
      expect(specifiers).toContainEqual({ specifier: "./value-comparator.js", line: 1 });
      expect(resolvesInsideAllowedRoots(FROM_CONTRADICT, "./value-comparator.js")).toBe(true);
    });
  ...
  it("sanity: the scan actually walked real files under lib/contradiction/, and excludes __tests__", () => {
    const files = listNonTestSourceFiles(CONTRADICTION_ROOT);
    expect(files.length).toBeGreaterThanOrEqual(4);
    expect(files.some((f) => f.endsWith("contradict.ts"))).toBe(true);
    expect(files.some((f) => f.includes("__tests__"))).toBe(false);
  });

  it("sanity: this repo's OWN lib/contradiction/** source passes all three checks right now (a true positive on the real codebase, not just synthetic fixtures)", () => {
  ...
  describe("EXPLOIT REGRESSION (independent verification, round 5): syntax errors defeated the real parser's error recovery the same way earlier rounds defeated the tokenizer — the bug class MOVED, not closed, until diagnostics are checked", () => {
    ...
    it("does NOT overtighten: a legitimate relative re-export is still accepted, not merely un-flagged", () => {
      ...
      expect(resolvesInsideAllowedRoots(FROM_CONTRADICT, "./helper.js")).toBe(true);
    });
  });
});
